...
LDS_FILE			=	mak/$(PRODUCT_DEF)/sta$(XIPSIZE).lds
BIN_HEADER_TYPE         = 4
...
LDS_FILE			=	mak/$(PRODUCT_DEF)/ap$(XIPSIZE).lds
BIN_HEADER_TYPE         = 6
...
OUT_PATH			=	out$(XIPSIZE)/$(OPTION)
OBJ_PATH			=	obj$(XIPSIZE)/$(OPTION)
...
#*******************************************************************************
REQ_DIRS			=	$(OBJ_PATH) $(OUT_PATH) $(subst $(SRC_PATH)/,$(OBJ_PATH)/,$(sort $(dir $(SRC_FILES))))
REQ_DIRS			+=	$(subst $(API_PATH)/,$(OBJ_PATH)/,$(sort $(dir $(API_FILES))))
...
$(ASM_OBJS) : $(OBJ_PATH)/%.o : $(SRC_PATH)/%.S
	@echo Compiling $< ...
	@echo Compiling $< ... >> $(BUILD_LOG)
	@$(AS) -g $(BFLAGS) $(CFLAGS) $(TFLAGS) $(OFLAGS) $(BFLAGS) $(MFLAGS) $(DBGFLAGS) $(FUNCFLAGS) $(foreach i,$(INCLUDES),-I $i) -o $@ $< 2>> $(BUILD_LOG)
...
# Pattern for assembly dependence
$(ASM_DEPENDS) : $(OBJ_PATH)/%.d : $(SRC_PATH)/%.S
	@$(AS) -M $(foreach i,$(INCLUDES),-I $i) $< > $@.$$$$; \
	$(SED) 's,$(notdir $*)\.o,$(@:%.d=%.o),g' < $@.$$$$ > $@; \
	$(RM) $@.$$$$
...
#generate flash bin 
ifeq ($(BUILD_TYPE),0)
	@./flash_merge$(XIPSIZE).sh 3
endif	

ifeq ($(BUILD_TYPE),1)
	@./flash_merge$(XIPSIZE).sh 1
	@./flash_merge$(XIPSIZE).sh 3
endif	
	
ifeq ($(BUILD_TYPE),2)
	@./flash_merge$(XIPSIZE).sh 2
	@./flash_merge$(XIPSIZE).sh 3
endif	
